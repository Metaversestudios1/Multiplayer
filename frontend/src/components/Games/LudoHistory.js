...
const LudoHistory = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      let apiUrl = `${process.env.REACT_APP_BACKEND_URL}/api/getallludohistory?page=${page}&limit=${pageSize}&search=${search}`;
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center">
        <div className="text-2xl font-bold mx-2 my-8 px-4">Ludo History</div>
      </div>
      <div className="flex justify-between">
        <div className={`flex items-center`}>
          <NavLink to="/addludohistory">
      ...
      {noData && (
        <div className="text-center text-xl">
          Currently! There are no ludo history in the storage.
        </div>
      )}
