...
const Games = () => {
  ...
  const handleDelete = async (e, id) => {
    ...
    if (permissionOfDelete) {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/deletegame`,
        {
          method: "DELETE",
          ...
          <table className="w-full text-sm text-left border-2 border-gray-300">
            <thead className="text-xs uppercase bg-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 border-2">Game Description</th>
                  ...
                  <td className="px-6 py-4 font-medium border-2">
                    <div className="flex justify-center gap-2">
                      <CiEdit
                        ...
                        onClick={() => navigate(`/editgame/${item._id}`)}
                      />
                      <CiSettings
                        className="text-xl text-blue-500 hover:scale-110 cursor-pointer"
                        onClick={() =>
                          navigate(`/editgamesettings/${item._id}`)
                        }
                      />
                      <MdDelete
