  ...
  const fetchWins = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/getallleadboard?page=${page}&limit=${pageSize}&search=${search}&transactionType=withdraw`
      );
      const response = await res.json();
      console.log("leadboard rsponse:", response.result);
      if (response.success) {
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center">
        <div className="text-2xl font-bold mx-2 my-8 px-4">LeadBoard </div>
      </div>
          ...
          <table className="w-full text-sm text-left border-2 border-gray-300">
            <thead className="text-xs uppercase bg-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 border-2">Username</th>
                <th className="px-6 py-3 border-2">Total Coin Wins</th>
