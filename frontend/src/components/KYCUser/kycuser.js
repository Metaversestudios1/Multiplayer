...
const User = () => {
  ...
  const fetchData = async () => {
    ...
    if (response.success) {
      ...
      try {
        const usersWithusernameNames = await Promise.all(
          response.result.map(async (u) => {
            const user = await fetchuserName(u.user_id);
            return {
              ...u,
              user: user,
            };
        ...
        setUsers(usersWithusernameNames);
        //console.log("Full details with name:", usersWithusernameNames);
      } catch (error) {
                ...
                <tr key={item?._id} className="bg-white">
                  ...
                  <td className="px-6 py-4 font-medium border-2">
                    {item?.kycstatus === "pending" ? (
                      ...
                      <div className=" hover:bg-red-600  font-bold py-2 px-4 rounded">
                        Rejected
                      </div>
                    )}
                    &ensp;
