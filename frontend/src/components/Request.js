...
const Payment = () => {
      ...
      if (response.success) {
        const paymentWithuser = await Promise.all(
          response.result.map(async (users) => {
            ...
            const user_id = user.user_id; //changed from u_id to user_id
            return {
              ...users,
              username,
              user_id,
  ...
  const fetchKycRequests = async () => {
    ...
    try {
      //not working giving response scucess with empty array
      // const res = await fetch(
      //   `${process.env.REACT_APP_BACKEND_URL}/api/getAllUserBank?page=${page}&limit=${pageSize}&search=${search}&kycstatus=pending`
      // );
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/getAllUserBank`
      );
      const response = await res.json();
      //console.log("userbank details:", response.result);
      if (response.success) {
        const paymentWithuser = await Promise.all(
          response.result.map(async (users) => {
            const user = await fetchuserName(users.user_id);
            const username = user.username;
            const user_id = user.user_id; //u_id
            return {
              ...users,
              username,
              user_id,
        ...
        //console.log("kyc requsts:", paymentWithuser);
                ...
                {withdrawals.map((item, index) => (
                  <tr key={item._id} className="bg-white border-b">
                    ...
                    <td className="px-6 py-4 font-medium border-2">
                      <CiEdit
                        className="text-xl text-blue-500 hover:scale-110 cursor-pointer"
                        onClick={() => navigate(`/withdrawhistory/${item._id}`)}
                      />
                    </td>
                  </tr>
                ))}
