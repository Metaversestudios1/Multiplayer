...
const BankDetails = () => {
  ...
  const fetchOldData = async () => {
    const res = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/api/getbankdetails`
    );
    const response = await res.json();
    console.log("Bank Details:", response.result[0]);
    if (response.success) {
      setData({
        ...data,
        bankName: response?.result[0]?.bankName,
        accountNo: response?.result[0]?.accountNo,
        accountholderhame: response?.result[0]?.accountholdername,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/updatebankdetails`,
        {
          method: "PUT",
