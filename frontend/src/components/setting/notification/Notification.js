...
const Notification = () => {
  ...
  const fetchWins = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/allnotification?page=${page}&limit=${pageSize}&search=${search}&transactionType=withdraw`
      );
      const response = await res.json();
      //console.log("notification rsponse:", response.result);
      if (response.success) {
  ...
  const handleDelete = async (e, id) => {
    ...
    if (permissionOfDelete) {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/deletenotification`,
        {
          method: "DELETE",
      ...
      const response = await res.json();
      if (response.success) {
        toast.success("Notification is deleted Successfully!", {
          position: "top-right",
          autoClose: 1000,
    ...
    <div className="relative">
          ...
          <table className="w-full text-sm text-left border-2 border-gray-300">
            <thead className="text-xs uppercase bg-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 border-2">Is Global</th>
                <th className="px-6 py-3 border-2">Message</th>
                <th className="px-6 py-3 border-2">Type</th>
                <th className="px-6 py-3 border-2">Added At</th>
                <th className="px-6 py-3 border-2">Send To</th>
      ...
      <div className="flex justify-end">
        ...
        <button
          ...
          disabled={wins.length < pageSize || startIndex + pageSize >= count}
          className={`bg-blue-800 text-white p-2 m-5 text-sm rounded-lg ${
