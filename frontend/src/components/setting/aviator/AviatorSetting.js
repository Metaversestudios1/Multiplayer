...
const AviatorSetting = () => {
  ...
  const fetchOldData = async () => {
    ...
    if (response.success) {
      setData({
        ...
        gameStartTime: response.result.gameStartTime
          ? response?.result[0]?.gameStartTime.split("T")[0]
          : null,
  ...
  const validateSettingForm = () => {
    $("#settingform").validate({
      ...
      messages: {
        ...
        withdrawalFee: {
          required:
            "Please enter Withdrawal fee, it will be consider as in percentage",
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/updatesetting`,
        {
          method: "PUT",
