...
const gameLogic = async (io) => {
  ...
  const saveGameHistory = async (gameResults) => {
    ...
    try {
      ...
      await newHistory.save();
      if (gameResults.totalWinningAmount > 0) {
        // Update user balance after game ends
        for (const userResult of gameResults.userResults) {
          const user = await User.findById(userResult.userId);
          if (user) {
            user.balance =
              (user.balance || 0) + parseFloat(userResult.winnings);
            await user.save();
          }
        }
      }
      users = {}; // Reset users after saving game history
    } catch (error) {
      console.error("Error saving game history:", error);
    }
  };
    ...
    setTimeout(() => {
      ...
      let gameInterval = setInterval(() => {
        ...
        if (multiplier >= crashPoint) {
          ...
          // Reset user data after the game ends
          setTimeout(() => {
            users = {}; // Clear user data
